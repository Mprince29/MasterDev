    ...
    <main className="relative min-h-screen text-white">
      ...
      <div className="max-w-6xl mx-auto pt-32 px-4 sm:px-6 mb-8 sm:mb-16">
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <motion.h1
            ...
            className="text-2xl sm:text-4xl md:text-5xl font-bold mb-4 sm:mb-12 bg-gradient-to-r from-blue-400 via-blue-200 to-white text-transparent bg-clip-text"
          >
            Projects
          </motion.h1>
            ...
            {projects.map((project, index) => {
              ...
              return (
                <motion.div
                  key={project.id}
                  variants={itemVariants}
                  className="w-full flex flex-col lg:flex-row items-stretch animated-glow backdrop-blur-sm rounded-xl border border-white/10 shadow-lg hover:shadow-blue-500/40 hover:border-blue-500/30 transition-all duration-700 ease-out overflow-hidden h-[280px] sm:h-[300px] group cursor-pointer"
                  ...
                  <div className="flex-1 flex flex-col justify-between p-4 sm:p-6 lg:p-8">
                    <div className="space-y-3 flex-1">
                      <div className="flex items-start gap-3">
                        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold bg-gradient-to-r from-blue-400 via-blue-200 to-white text-transparent bg-clip-text leading-tight line-clamp-2 flex-1">
                          {project.title}
                        </h2>
                        {project.highlight && (
                          <span className="shrink-0 text-[10px] font-semibold text-blue-300 bg-blue-500/10 border border-blue-500/30 rounded px-2 py-0.5 mt-1">
                            {project.highlight}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-1.5 max-h-12 overflow-hidden">
                          {project.tags.slice(0, 4).map((tag) => (
                            <span key={tag} className="px-2 py-1 rounded-md bg-blue-500/10 text-blue-300 text-xs font-medium border border-blue-500/30 hover:bg-blue-500/20 transition-colors duration-200">
                              {tag}
                            </span>
                          ))}
      ...
      <footer className="bg-black/80 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-6">
          <span className="text-gray-400 text-sm">© {new Date().getFullYear()} Prince. All rights reserved.</span>
